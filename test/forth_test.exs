defmodule ForthTest do
  ...
  test "DUP operations" do
    assert {:ok, %{stack: [1, 1]}} = Forth.new()
    |> Forth.eval("1 DUP")

    assert {:ok, %{stack: [10, 30, 30]}} = Forth.new()
    |> Forth.eval("10 30 dup")

    assert {:ok, %{stack: [10, 10, 10, 10]}} = Forth.new()
    |> Forth.eval("10 DUP dup DUp")

    # dup failed
    assert {:error, "Not enough elements in stack for operaiton: DUP"} = Forth.new()
    |> Forth.eval("DUP")
  end

  test "DROP operations" do
    assert {:ok, %{stack: []}} = Forth.new()
    |> Forth.eval("1 drop")
    ...
    # drop failed
    assert {:error, "Not enough elements in stack for operaiton: DROP"} = Forth.new()
    |> Forth.eval("10 20 DROP DROP drop")
  end

  test "SWAP operations" do
    assert {:ok, %{stack: [2, 1]}} = Forth.new()
    |> Forth.eval("1 2 swap")

    assert {:ok, %{stack: [10, 20, 40, 30]}} = Forth.new()
    |> Forth.eval("10 20 30 40 SWAP")

    assert {:ok, %{stack: [20, 30, 40, 10]}} = Forth.new()
    |> Forth.eval("10 20 swap 30 swap 40 swap")

    # swap failed
    assert {:error, "Not enough elements in stack for operaiton: SWAP"} = Forth.new()
    |> Forth.eval("10 swap swap SWAP")
  end

  test "OVER operations" do
    assert {:ok, %{stack: [20, 30, 20]}} = Forth.new()
    |> Forth.eval("20 30 over")

    assert {:ok, %{stack: [10, 20, 10, 20, 10]}} = Forth.new()
    |> Forth.eval("10 20 over Over OVEr")

    # over failed
    assert {:error, "Not enough elements in stack for operaiton: OVER"} = Forth.new()
    |> Forth.eval("10 over")
  end
end
